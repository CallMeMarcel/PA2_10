...
// SubmitReview menerima input review dari user login
func (uc *UserController) SubmitReview(ctx *fiber.Ctx) error {
    cookie := ctx.Cookies("jwtUser")
    token, err := jwt.ParseWithClaims(cookie, &jwt.StandardClaims{}, func(t *jwt.Token) (interface{}, error) {
        return []byte(UserSecretKey), nil
    })
    ...
    if err := database.DB.Create(&review).Error; err != nil {
        return ctx.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"message": "Failed to save review"})
    }

    return ctx.JSON(fiber.Map{"message": "Review submitted", "review": review})
}

// GetAllReviews mengembalikan semua review
func (uc *UserController) GetAllReviews(ctx *fiber.Ctx) error {
    var reviews []models.Review
    if err := database.DB.Order("created_at desc").Find(&reviews).Error; err != nil {
        return ctx.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"message": "Error retrieving reviews"})
    }
